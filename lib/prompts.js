...
// Worked-example escape hatch (roadmap A3 / T-03).
// When a student asks to SEE a fully worked example, the tutor must demonstrate on a NEW,
// clearly-different parallel problem — and must never solve or reveal the answer to the
// student's own submitted problem. This guard is appended to the system prompt for that
// one turn. Kept here (not inline in the handler) so the anti-cheat invariant is unit-tested.
export function getWorkedExampleGuard() {
  return `WORKED-EXAMPLE MODE — the student has asked to SEE a fully worked example.
- Invent a NEW practice problem of the SAME type, skill, and difficulty as the one the student is currently working on, but with CLEARLY DIFFERENT numbers and a different context. It must NOT be the student's own problem in disguise.
- Solve THAT new example completely, step by step, narrating your reasoning so the method is clear.
- ABSOLUTELY DO NOT solve, finish, or reveal the final answer to the student's OWN submitted problem — not even partially. The worked example is a parallel demonstration only.
- End by inviting the student to now try their own problem using the same method.`;
}
