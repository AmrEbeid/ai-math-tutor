...
Stage 1 planning is closed as a readiness package, the Stage 2 plan is drafted, and the
**STAGE1-1 working-tree cleanup/commit plan is now drafted**
(`SPEC-STAGE1-1-working-tree-cleanup-commit-plan.md`): the 14 uncommitted source files are
classified (Stage 0 `api/*` security/payment/auth vs A0.5 `public/*` onboarding), the
untracked `docs/` tree is inventoried, and an explicit file-listed commit plan (C1–C3 docs
→ C4 Stage 0 → C5 A0.5) with review gates is proposed. The next priority is **GPT/user
review of STAGE1-1**, then approve either a docs-only commit (`STAGE1-1A`) or a deeper
source-diff review (`STAGE1-1R`). **No commits made; no source changed.** No implementation,
installs, migrations, React/Vite, or Stage 2 code without approval.
...
* **Stage 2 is blocked** pending Stage 1 implementation readiness; its plan is drafted but
  ...
  hard gates).
* Pre-existing uncommitted Stage 0/A0.5 `api/*`+`public/*` diffs are unresolved; the
  cleanup/commit plan is drafted (STAGE1-1) but **no commit is authorized yet** — Stage 0
  source (C4) needs review + manual Lemon Squeezy verification; A0.5 source (C5) needs flow
  verification; `PROJECT_BRIEF.md` is held out pending drift reconciliation.
...
Stage 1 planning + the Stage 2 master plan + the **STAGE1-1 working-tree cleanup/commit
plan** are drafted. The next safe action is **GPT/user review of
`docs/specs/SPEC-STAGE1-1-working-tree-cleanup-commit-plan.md`**; then approve either
**STAGE1-1A — Review and Commit Docs Only** (commit docs C1–C3 with explicit file lists)
or **STAGE1-1R — Source Diff Review Before Commit** (deeper Stage 0/A0.5 diff review). The
pre-existing uncommitted Stage 0 `api/*` + A0.5 `public/*` diffs remain **uncommitted and
unmodified**; `PROJECT_BRIEF.md` is held out (drift). Stage 2's first slice (when reached)
is the read-only **STAGE2-A** audit. Do not commit, stage source, reset/revert/stash, start
