...
| STAGE1 | Tests, Schema Documentation & Tooling Evaluation    | **Complete locally** / blocked externally on live gates | `SPEC-STAGE1-LOCAL-ACCEPTANCE.md` | External gate review (apply migration 002; manual LS verify; deploy) | Medium | 2026-06-03   | Source committed in clean slices C4a `15d62f5` / C4b `8a60d2d` / C4c `b3f043f` / C5 `f0b4c87`; env validation + `.env.example` `2a32f76`; test baseline `84d2084` (22 tests, 0 installs). Migration `002` (webhook idempotency unique index + processed_webhooks + notifications.type CHECK fix for payment_failed/child_distress/exam_completed/personal_info_shared) **created, NOT applied**. Live gates: apply migration, manual LS replay, deploy, RLS hardening, token storage. No React/Vite. |
...
| STAGE2-LOCAL | Stage 2 static child chat UX (local) | Done | commit 4360957 | Additive `public/app.html` a11y/hint-first/session-expired/error UX + 9 smoke tests (31 total). No React/Vite/providers/token-storage change/deploy. Created SPEC-STAGE2-LOCAL-ACCEPTANCE. Deeper items deferred. |
