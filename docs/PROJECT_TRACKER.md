...
| -------- | ----------------------------------- | ------------------- | ------------------- | ---------------------------------------- |
| BRAND-ROLLOUT-REMAINING | Finish the teal Spark brand across the remaining 9 pages (revamp polish) | **Built** — PR open → `main` | branch `feat/brand-rollout-remaining-pages` | Completes the staged brand rollout: applied the same surgical **`--color-accent` → teal `--brand-*`** override (light + dark) to `pricing.html`, `dashboard.html`, `exam-prep.html`, `app.html`, and the legal/safety pages (`privacy`,`terms`,`gdpr`,`refund`,`safety`) — one `<style id="brand-rollout">` block per page inserted before `</head>`. Now the **whole app is teal-consistent** (favicon/app-icon were already teal from PR #19). **No JS/markup/copy changed** (dashboard's 💡 is a notification icon, left as-is); `sw.js` `v12→v13`. Verified pricing page screenshot (teal); `npm test` 123/1 green (pricing card/14-day + safety copy assertions intact). **Frontend CSS only; no auth/payment/schema/migration/deploy.** |
| AUTH-FLOW-DESIGN | Login/signup flow redesign (revamp sub-project 3/3, flow half) — neutral age-gate, VPC, mobile-first consent | **Done (design-only) — MERGED to `main` 2026-06-17** (PR #22, merge `c4ec461`) | branch `docs/auth-flow-design`; spec `docs/superpowers/specs/2026-06-17-login-signup-flow-design.md` | branch `docs/auth-flow-design`; spec `docs/superpowers/specs/2026-06-17-login-signup-flow-design.md` | Design for the signup/consent funnel grounded in the verified research + COPPA: **neutral age-gate** (plain DOB/grade, no nudge), **explicit logged consent screen** (not a buried checkbox), **VPC method legal-gated** (keep card-with-notice for paid; design an **email-plus** path to protect MENA conversion since card-VPC converts poorly there), **no child PII before a consent record**, **"Grown-Ups Only"** boundary, mobile-first. References COPPA research + A0.5 + trial-enforcement; consent-ledger = gated migration. **Docs-only; every flow change is a hard gate (auth/consent/schema) + legal sign-off before build.** |
| AUTH-PAGES-REBRAND | Login / child-login / verify-email visual rebrand to teal Spark (revamp sub-project 3/3, visual half) | **Done — MERGED to `main` 2026-06-17** (PR #21, merge `0fc7db1`) | branch `feat/auth-pages-rebrand` | Surgical, **visual-only** rebrand of the 3 auth pages to the new brand — **all Supabase auth / OTP / child-login JS, forms, and IDs untouched**. Routed each page's `--color-accent` → teal `--brand-*` for **both light + dark** (one override block per page), added the inline **Spark** logo (replaces the 💡/🧠/✉ emojis in the wordmarks), teal primary buttons (pill), teal-tinted plan box, SVG favicon. `sw.js` `v11→v12`. Verified via headless-Chrome screenshots (all 3 pages, dark); `npm test` 123/1 green (`frontend-copy` login card/14-day disclosure intact). **Frontend CSS/markup only; no auth/payment/schema/migration/deploy.** Companion **flow design** (neutral age-gate, parent-verify→child, mobile-first consent) is a separate design doc — auth-logic build stays gated. |
